//! Configuration for MQTT source connector.
...
impl MqttConfig {
    ...
    pub fn from_env() -> ConnectorResult<Self> {
        let broker_host = std::env::var("MQTT_BROKER_HOST")
            .map_err(|_| danube_connect_core::ConnectorError::config("MQTT_BROKER_HOST is required"))?;
        ...
        let client_id = std::env::var("MQTT_CLIENT_ID")
            .map_err(|_| danube_connect_core::ConnectorError::config("MQTT_CLIENT_ID is required"))?;
        ...
        let topic_mappings: Vec<TopicMapping> = mqtt_topics
            ...
            .map(|topic| {
                ...
                let danube_topic = danube_topic
                    .clone()
                    .unwrap_or_else(|| format!("/mqtt{}", mqtt_topic.replace('#', "all").replace('+', "any")));
